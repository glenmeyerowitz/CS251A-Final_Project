...
Layer *createLayer(int nodeCount, int weightCount){
    ...
    int o;
    for (o=0;o<weightCount;o++) dn->weights[o] = 0; // will be initialized later
    
    uint8_t *sbptr = (uint8_t*) l->nodes;     // single byte pointer
    
    // copy the default cell to all cell positions in the layer
    int i;
    for (i=0;i<nodeCount;i++) memcpy(sbptr+(i*nodeSize),dn,nodeSize);
...
void initWeights(Network *nn, LayerType ltype){
    ...
    int o;
    for (o=0; o<l->ncount;o++){
    
        Node *n = (Node *)sbptr;
        
        int i;
        for (i=0; i<n->wcount; i++){
...
void saveNetworkWeights(Network *nn){
    ...
    Layer *outputLayer = getLayer(nn, OUTPUT);
    int inpLNC = inputLayer->ncount;
    int hidLNC = hiddenLayer->ncount;
    int outLNC = outputLayer->ncount;
    // Save values to file for the INPUT layer and its nodes
    if (inpLNC>0){
        int n;
        for(n=0; n<inpLNC; n++) {
            Node *node = getNode(inputLayer, n);
    ...
    // Save values to file for the HIDDEN layer and its nodes
    if (hidLNC>0){
        int n;
        for(n=0; n<hidLNC; n++) {
            Node *node = getNode(hiddenLayer, n);
            int connCount = node->wcount;
            int c;
            for (c=0; c<connCount; c++){
                double w = node->weights[c];
    ...
    // Save values to file for the OUTPUT layer and its nodes
    if (outLNC>0){
        int n;
        for(n=0; n<outLNC; n++) {
            Node *node = getNode(outputLayer, n);
            int connCount = node->wcount;
            int c;
            for (c=0; c<connCount; c++){
                double w = node->weights[c];
...
void loadNetworkWeights(Network *nn){
    ...
    int hidLNC = hiddenLayer->ncount;
    int outLNC = outputLayer->ncount;
    // Load the values from file for the INPUT layer and its nodes
    if(inpLNC>0){
        int n;
        for(n=0; n<inpLNC; n++){
            Node *node = getNode(inputLayer, n);
            ...
            int c;
            for (c=0; c<connCount; c++){
                fgets(str, 60, inputFile);
                node->weights[c] = atof(str);
            }
        }
    }
    // Load the values from file for the HIDDEN layer and its nodes
    if(hidLNC>0){
        int n;
        for(n=0; n<hidLNC; n++){
            Node *node = getNode(hiddenLayer, n);
    ...
    // Load the values from file for the OUTPUT layer and its nodes
    if(outLNC>0){
        int n;
        for(n=0; n<outLNC; n++){
            Node *node = getNode(outputLayer, n);
    ...
    // Close the input file after loading
    fclose(inputFile);
}
